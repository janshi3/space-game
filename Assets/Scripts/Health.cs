...
public class Health : MonoBehaviour
{
    ...
    private void OnCollisionEnter(Collision collisionInfo)
    {
        if (collisionInfo.collider.tag == "Laser")
        {
            ...
            if (collisionInfo.collider.GetComponent<ProjectileTimer>().shotBy == 0 && gameObject.tag == "Ally")
            {
                anger += 1;
            }
    ...
    void Update()
    {
        ...
        if (currentHealth <= 0)
        {
            ...
            else
            {
                ...
                if (gameObject.GetComponent<AI_Movement>().shipType == "Multishot")
                {
                    if (Random.Range(0, 3) == 0)
                    {
                        var powerUp = Instantiate(powerUps[2]);
                        powerUp.transform.position = transform.position;
                        Vector3 movement = new Vector3(Random.Range(0, 100), 0, Random.Range(0, 100));
                        powerUp.GetComponent<Rigidbody>().AddForce(movement);
                        Vector3 newRotation = new Vector3(0, Random.Range(0, 100), 0);
    ...
    public void GetHit(int attackerID)
    {
        currentHealth -= 200;
        ...
        if (attackerID == 0 && gameObject.tag == "Ally")
        {
            anger += 1;
        }
